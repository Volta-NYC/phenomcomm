...
for (const width of [1440, 768, 390, 320]) {
  test(`All pages render without overflow or missing assets at ${width}px`, async ({ page }) => {
    ...
    for (const route of routes) {
      ...
      // Full-page captures bypass the observer, so settle any reveal the fast scroll skipped.
      await page.evaluate(() =>
      ...
      if (width !== 320)
        await page.screenshot({
          path: `artifacts/qa/${route.replaceAll("/", "") || "home"}-${width}.png`,
          fullPage: true,
        });
    }
...
test("Mobile navigation opens, traps focus, closes on Escape, and navigates", async ({ page }) => {
  ...
  await page.keyboard.press("Shift+Tab");
  await expect(
    page.getByRole("navigation", { name: "Mobile navigation" }).getByRole("link", { name: "Schedule a consult" }),
  ).toBeFocused();
...
test("Book companion stays unlisted and noindex; local PDFs and icons resolve", async ({ page, request }) => {
  ...
  for (const path of ["/icon.png", "/apple-icon.png", "/icon-192.png", "/icon-512.png", "/opengraph-image.png"])
...
test("Workbook form validates, handles failures, and unlocks on confirmed success", async ({ page }) => {
  ...
  await page.route("**/api/workbook", (route) =>
    route.fulfill({
      status: 502,
      contentType: "application/json",
      body: JSON.stringify({ error: "The email service is temporarily unavailable." }),
    }),
  );
  await page.getByRole("button", { name: "Get the free workbook" }).click();
  ...
  await page.getByRole("button", { name: "Get the free workbook" }).click();
  await expect(page.getByRole("link", { name: "Download your workbook" })).toHaveAttribute(
    "href",
    "/downloads/upgrade-framework-workbook.pdf",
  );
});
...
test("Workbook API rejects malformed and cross-origin requests before provider contact", async ({ request }) => {
  expect(
    (
      await request.post("/api/workbook", {
  ...
  expect(
    (
      await request.post("/api/workbook", {
        data: { name: "Test", email: "test@example.com", website: "spam" },
        headers: { Origin: "http://localhost:3000" },
      })
    ).status(),
  ).toBe(400);
